...
public class CcsServerExceptionMapper {
  ...
  public static RuntimeException getCcsException(final int status, final String message, final Throwable throwable) {
    ...
    if (status < Response.Status.INTERNAL_SERVER_ERROR.getStatusCode()) {
      return switch (status) {
        case 403 ->
            (throwable == null) ? new CcsNotAllowedException(realMsg) : new CcsNotAllowedException(realMsg, throwable);
        case 404 ->
  ...
  public Uni<Response> handleWebClientApplicationException(final WebClientApplicationException exception) {
