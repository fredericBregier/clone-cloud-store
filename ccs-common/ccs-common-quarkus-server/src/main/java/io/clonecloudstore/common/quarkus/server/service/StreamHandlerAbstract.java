...
public abstract class StreamHandlerAbstract<I, O> {
  ...
  public Response pull() throws ServerStreamHandlerResponseException {
    try {
      preparePull();
  ...
  public Response upload(final InputStream inputStream) throws ServerStreamHandlerResponseException {
    try {
      closer.add(inputStream);
      LOGGER.debugf("START UPLOAD %s", businessIn);
      final var inputStreamFinal =
          prepareUpload(inputStream, ParametersChecker.isEmpty(getOriginalHash()) && checkDigestToCompute(businessIn));
  ...
  private void internalError(final Throwable cause) {
    if (cause instanceof CcsClientGenericException || cause instanceof CcsServerGenericException ||
        cause instanceof CcsWithStatusException || cause instanceof ServerStreamHandlerResponseException) {
      LOGGER.infof("Exception: %s", cause.getMessage());
    } else {
      LOGGER.error("Unattended Exception: ", cause);
    }
  }
  ...
  private MultipleActionsInputStream prepareUpload(final InputStream inputStream, final boolean digestNeeded)
      throws ServerStreamHandlerResponseException {
    try {
      MultipleActionsInputStream newInputStream =
          MultipleActionsInputStream.create(inputStream, digestNeeded); // NOSONAR intentional
  ...
  protected void sendError(final int status, final Exception cause) throws ServerStreamHandlerResponseException {
    internalError(cause);
    sendErrorInternal(Response.Status.fromStatusCode(status));
  }

  protected void sendError(final Response.Status status, final Throwable t)
      throws ServerStreamHandlerResponseException {
    internalError(t);
    sendErrorInternal(status);
  }

  private void sendErrorInternal(final Response.Status status) throws ServerStreamHandlerResponseException {
    ...
    final var exception = getServerStreamHandlerResponseException(status);
    LOGGER.debug(status, exception);
    throw exception;
  }

  protected ServerStreamHandlerResponseException getServerStreamHandlerResponseException(final Response.Status status) {
    ...
    return new ServerStreamHandlerResponseException(responseBuilder.build(),
    ...
    if (exc != null) {
      ...
      switch (exc) {
        case final CcsWithStatusException cse ->
            throw CcsServerExceptionMapper.getCcsException(cse.getStatus(), cse.getMessage(), cse);
        case final CcsClientGenericException be -> throw be;
        case final CcsServerGenericException be -> throw be;
        case final ServerStreamHandlerResponseException ne -> {
