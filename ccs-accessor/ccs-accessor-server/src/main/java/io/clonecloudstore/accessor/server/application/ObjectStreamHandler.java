...
import io.clonecloudstore.accessor.server.commons.AbstractObjectStreamHandler;
...
@RequestScoped
public class ObjectStreamHandler extends AbstractObjectStreamHandler {
  private static final Logger LOGGER = Logger.getLogger(ObjectStreamHandler.class);
  ...
  protected ObjectStreamHandler(final AccessorObjectService service) {
    super(service);
  }
  ...
  protected InputStream getPullInputStream(final AccessorObject object) {
    try {
      if (isListing) {
        return service.filterObjects(object.getBucket(), filter, clientId, external);
      }
      ...
      var inputStream = getFilesystemInputStream(object);
      if (inputStream != null) {
        return inputStream;
      }
      if (external && AccessorProperties.isRemoteRead() && remoteTargetId.get() != null) {
        ...
        if (AccessorProperties.isFixOnAbsent()) {
          var objectChecked = checked.get();
          ((AccessorObjectService) service).generateReplicationOrderForObject(object.getBucket(), object.getName(),
              clientId, getOpId(), remoteTargetId.get(), objectChecked.getSize(), objectChecked.getHash());
        }
  ...
  private InputStream getFilesystemInputStream(final AccessorObject object) {
    if (AccessorProperties.isStoreActive()) {
      try {
        return filesystemHandler.readContent(object.getBucket(), object.getName());
  ...
  protected Map<String, String> getHeaderError(final AccessorObject object, final int status) {
    try {
      final var currentStatus = service.getObjectInfo(object.getBucket(), object.getName(), clientId).getStatus();
      ...
      // And If UPLOAD or UNKNOWN or NULL, can change status
