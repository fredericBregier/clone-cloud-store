...
import static io.clonecloudstore.accessor.config.AccessorConstants.Api.TAG_OBJECT;
 ...
 */
@Path(INTERNAL_ROOT)
public class AccessorObjectInternalResource
    extends StreamServiceAbstract<AccessorObject, AccessorObject, ObjectNativeStreamHandler> {
  private static final Logger LOGGER = Logger.getLogger(AccessorObjectInternalResource.class);
  ...
  @Tag(name = TAG_INTERNAL + TAG_OBJECT)
  @Path("{bucketName}")
  @PUT
  ...
  @Parameters({
      ...
      @Parameter(name = FILTER_NAME_PREFIX, description = "Operation ID", in = ParameterIn.HEADER, schema =
      @Schema(type = SchemaType.STRING), required = false),
      @Parameter(name = FILTER_STATUSES, description = "Operation ID", in = ParameterIn.HEADER, schema =
      @Schema(type = SchemaType.STRING), required = false),
      @Parameter(name = FILTER_CREATION_BEFORE, description = "Operation ID", in = ParameterIn.HEADER, schema =
      @Schema(type = SchemaType.STRING), required = false),
      @Parameter(name = FILTER_CREATION_AFTER, description = "Operation ID", in = ParameterIn.HEADER, schema =
      @Schema(type = SchemaType.STRING), required = false),
      @Parameter(name = FILTER_EXPIRES_BEFORE, description = "Operation ID", in = ParameterIn.HEADER, schema =
      @Schema(type = SchemaType.STRING), required = false),
      @Parameter(name = FILTER_EXPIRES_AFTER, description = "Operation ID", in = ParameterIn.HEADER, schema =
      @Schema(type = SchemaType.STRING), required = false),
      @Parameter(name = FILTER_SIZE_LT, description = "Operation ID", in = ParameterIn.HEADER, schema = @Schema(type
          = SchemaType.INTEGER), required = false),
      @Parameter(name = FILTER_SIZE_GT, description = "Operation ID", in = ParameterIn.HEADER, schema = @Schema(type
          = SchemaType.INTEGER), required = false),
      @Parameter(name = FILTER_METADATA_EQ, description = "Operation ID", in = ParameterIn.HEADER, schema =
      @Schema(type = SchemaType.STRING), required = false)})
  ...
  @Blocking
  public Uni<Response> listObjects(@RestPath final String bucketName, @HeaderParam(ACCEPT) final String acceptHeader,
                                   ...
                                   @Parameter(name = AccessorConstants.Api.X_CLIENT_ID, description = "Client ID",
                                       ...
                                       = true) @HeaderParam(X_CLIENT_ID) final String clientId,
                                   ...
                                   @HeaderParam(FILTER_NAME_PREFIX) final String xNamePrefix,
                                   @HeaderParam(FILTER_STATUSES) final String xStatuses,
                                   @HeaderParam(FILTER_CREATION_BEFORE) final String xCreationBefore,
                                   @HeaderParam(FILTER_CREATION_AFTER) final String xCreationAfter,
                                   @HeaderParam(FILTER_EXPIRES_BEFORE) final String xExpiresBefore,
                                   @HeaderParam(FILTER_EXPIRES_AFTER) final String xExpiresAfter,
                                   @DefaultValue("0") @HeaderParam(FILTER_SIZE_LT) final long xSizeLt,
                                   @DefaultValue("0") @HeaderParam(FILTER_SIZE_GT) final long xSizeGt,
                                   @HeaderParam(FILTER_METADATA_EQ) final String xMetadataEq,
                                   final HttpServerRequest request, @Context final Closer closer) {
    final var finalBucketName = DaoAccessorBucketRepository.getRealBucketName(clientId, bucketName, false);
  ...
  public Uni<Response> getObject(@RestPath final String bucketName, @RestPath final String objectName,
                                 ...
                                 @Parameter(name = AccessorConstants.Api.X_CLIENT_ID, description = "Client ID", in =
                                     ParameterIn.HEADER, schema = @Schema(type = SchemaType.STRING), required = true) @HeaderParam(X_CLIENT_ID) final String clientId,
                                 @Parameter(name = X_OP_ID, description = "Operation ID", in = ParameterIn.HEADER,
                                     schema = @Schema(type = SchemaType.STRING), required = false) @HeaderParam(X_OP_ID) final String opId,
                                 final HttpServerRequest request, @Context final Closer closer) {
    final var finalBucketName = DaoAccessorBucketRepository.getRealBucketName(clientId, bucketName, false);
