...
import io.clonecloudstore.accessor.server.simple.application.ObjectNativeStreamHandler;
...
import static io.clonecloudstore.accessor.server.simple.resource.AccessorBucketResource.getRealBucketName;
...
public class AccessorObjectResource
  ...
  public Uni<Response> listObjects(@PathParam("bucketName") final String bucketName,
    ...
    final var finalBucketName = getRealBucketName(clientId, bucketName);
  ...
  public Uni<Response> checkObjectOrDirectory(@PathParam("bucketName") final String bucketName,
    ...
    return Uni.createFrom().emitter(em -> {
      ...
      try {
        final var storageType = service.objectOrDirectoryExists(finalBucketName, finalObjectName);
  ...
  public Uni<Response> deleteObject(@PathParam("bucketName") final String bucketName,
    ...
    return Uni.createFrom().emitter(em -> {
      ...
      LOGGER.debugf(BUCKETNAME_OBJECT, finalBucketName, finalObjectName);
      try {
        service.deleteObject(finalBucketName, finalObjectName);
