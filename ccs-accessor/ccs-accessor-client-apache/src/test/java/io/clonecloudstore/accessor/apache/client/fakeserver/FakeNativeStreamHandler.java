...
public class FakeNativeStreamHandler extends FakeNativeStreamHandlerAbstract<AccessorObject, AccessorObject> {
  ...
  protected Map<String, String> getHeaderPullInputStream(final AccessorObject apiBusinessIn)
    ...
    final Map<String, String> mapData = new HashMap<>(apiBusinessIn.getMetadata());
    if (apiBusinessIn.getMetadata().isEmpty()) {
      mapData.put("key1", "value1");
    }
    AccessorHeaderDtoConverter.objectToMap(
        getBusinessOutForPushAnswer(apiBusinessIn, "hash", FakeObjectService.length).setId(GuidLike.getGuid())
            .setMetadata(mapData).setExpires(Instant.MAX), map);
    return map;
  }
  ...
  protected AccessorObject getBusinessOutForPushAnswer(final AccessorObject apiBusinessIn, final String finalHash,
    ...
    return new AccessorObject().setBucket(apiBusinessIn.getBucket()).setId(GuidLike.getGuid())
