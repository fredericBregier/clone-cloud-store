...
@Dependent
public abstract class FakeStreamHandlerAbstract extends AbstractObjectStreamHandler {
  ...
  protected void checkPushAble(final AccessorObject object, final MultipleActionsInputStream inputStream) {
    if (FakeCommonObjectResourceHelper.errorCode > 0) {
      throw CcsServerExceptionMapper.getCcsException(FakeCommonObjectResourceHelper.errorCode);
    }
    try {
  ...
  protected boolean internalCheckPullable(final AccessorObject object) {
    ...
    try {
      ...
      if (storageObject != null) {
        checked.set(AbstractPublicObjectHelper.getFromStorageObject(storageObject)
            .setSite(FakeCommonBucketResourceHelper.site));
