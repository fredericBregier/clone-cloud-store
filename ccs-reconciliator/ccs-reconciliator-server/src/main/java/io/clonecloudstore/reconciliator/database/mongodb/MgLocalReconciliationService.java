...
public class MgLocalReconciliationService implements LocalReconciliationService {
  ...
  /**
   * @return the previous if any Request on same bucket, clientId and fromSite
   * @throws CcsDbException if any issue
   */
  public DaoRequest step2GetLastPrevious(final DaoRequest request) throws CcsDbException {
    ...
    while (daoPreviousIterator.hasNext()) {
      ...
      if (previous == null) {
        ...
        if (previous.getStart() == null ||
            newOne.getStart() != null && previous.getStart().isBefore(newOne.getStart())) {
  ...
  public void importActions(final DaoRequest daoRequest, final Iterator<ReconciliationSitesAction> iterator)
    ...
    requestRepository.update(DbQuery.idEquals(daoRequest.getId()),
        new DbUpdate().set(DaoRequestRepository.ACTIONS, daoRequest.getActions())
