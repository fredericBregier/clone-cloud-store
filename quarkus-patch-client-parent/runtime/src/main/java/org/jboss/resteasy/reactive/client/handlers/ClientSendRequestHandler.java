...
public class ClientSendRequestHandler implements ClientRestHandler {
  ...
  public ClientSendRequestHandler(int maxChunkSize, boolean followRedirects, LoggingScope loggingScope,
                                  ClientLogger logger, Map<Class<?>, MultipartResponseData> multipartResponseDataMap) {
    ...
    future.subscribe().with(new Consumer<>() {
      ...
      public void accept(HttpClientRequest httpClientRequest) {
        ...
        if (requestContext.isMultipart()) {
          ...
          try {
            actualEntity =
                ClientSendRequestHandler.this.setMultipartHeadersAndPrepareBody(httpClientRequest, requestContext);
          ...
          vertx.fileSystem().open(filePathToUpload, new OpenOptions().setRead(true).setWrite(false), new Handler<>() {
            @Override
          ...
          try {
            actualEntity =
                ClientSendRequestHandler.this.setRequestHeadersAndPrepareBody(httpClientRequest, requestContext);
          } catch (Throwable e) {
            requestContext.resume(e);
            return;
          }
      ...
      public void handle(HttpClientResponse clientResponse) {
        try {
          ...
          if (isResponseMultipart(requestContext)) {
            QuarkusMultipartResponseDecoder multipartDecoder = new QuarkusMultipartResponseDecoder(clientResponse);
          ...
          } else if (!requestContext.isRegisterBodyHandler() && (Response.Status.Family.familyOf(status) ==
              Response.Status.Family.SUCCESSFUL)) { // we force the registration of a body handler if there was an
            // error, so we can ensure the body can be read
            ...
            if (requestContext.isFileDownload()) {
              ...
              vertx.fileSystem().createTempFile("rest-client", "", new Handler<>() {
                ...
                public void handle(AsyncResult<String> tempFileCreation) {
                  ...
                  vertx.fileSystem().open(tmpFilePath, new OpenOptions().setWrite(true), new Handler<>() {
                    @Override
                    public void handle(AsyncResult<AsyncFile> asyncFileOpened) {
                      ...
                      final Pump downloadPump = Pump.pump(clientResponse, tmpAsyncFile);
                      downloadPump.start();
                        ...
                        public void handle(Void event) {
                          tmpAsyncFile.flush(new Handler<>() {
                            public void handle(AsyncResult<Void> flushed) {
                              if (flushed.failed()) {
                                reportFinish(flushed.cause(), requestContext);
  ...
  public Uni<HttpClientRequest> createRequest(RestClientRequestContext state) {
    ...
    requestOptions = Uni.createFrom().item(new RequestOptions().setHost(uri.getHost()).setPort(port).setSsl(isHttps));

    return requestOptions.onItem().transform(r -> r.setMethod(HttpMethod.valueOf(state.getHttpMethod()))
        .setURI(uri.getRawPath() + (uri.getRawQuery() == null ? "" : "?" + uri.getRawQuery()))
        .setFollowRedirects(followRedirects)).onItem().invoke(r -> {
  ...
  private QuarkusMultipartFormUpload setMultipartHeadersAndPrepareBody(HttpClientRequest httpClientRequest,
    ...
    QuarkusMultipartFormUpload multipartFormUpload =
        new QuarkusMultipartFormUpload(Vertx.currentContext(), multipartForm, true, maxChunkSize, mode);
